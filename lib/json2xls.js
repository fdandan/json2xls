...
transform.prepareJson = function(jsons,config) {
    // init json data, format to multi sheet
    var jsonArr = [];
    if (jsons instanceof Array) {
        if (jsons[0] instanceof Array) {
            jsonArr = jsons;
        } else {
            jsonArr.push(jsons);
        }
    } else {
        jsonArr.push([].concat(jsons));
    }
    var ret = jsonArr.map(function(json, index) {
        var confs = [];
        var res = {};
        var conf = (config && config[index])||{};
        var fields = conf.fields || Object.keys(json[0]||{});
        var types = [];
        ...
        res.cols = fields.map(function(key,i) {
            return {
                caption: key,
                type: getType(json[0][key],types[i]),
        ...
        res = JSON.parse(JSON.stringify(res));   //clone
        res.name = 'sheet'+ index;
        return res;
    })
    return ret;
};
